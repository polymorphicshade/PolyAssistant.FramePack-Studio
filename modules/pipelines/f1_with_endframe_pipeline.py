"""
F1 with Endframe pipeline class for FramePack Studio.
This pipeline handles the "F1 with Endframe" model type.
"""
...
class F1WithEndframePipeline(F1Pipeline):
    """Pipeline for F1 with Endframe generation type."""
    
    def prepare_parameters(self, job_params):
        """
        Prepare parameters for the F1 with Endframe generation job.
        ...
        processed_params['model_type'] = "F1 with Endframe"
        
        return processed_params
    
    def validate_parameters(self, job_params):
        """
        Validate parameters for the F1 with Endframe generation job.
    ...
    def preprocess_inputs(self, job_params):
        """
        Preprocess input images for the F1 with Endframe generation type.
    ...
    def create_metadata(self, job_params, job_id):
        """
        Create metadata for the F1 with Endframe generation.
        ...
        # Create a placeholder image for the F1 with Endframe model
        height = job_params.get('height', 640)
        width = job_params.get('width', 640)
        placeholder_img = Image.new('RGB', (width, height), (0, 128, 128))  # Teal for F1 with Endframe
        ...
        metadata_dict = {
            ...
            "resolutionW": job_params.get('resolutionW', 640),
            "resolutionH": job_params.get('resolutionH', 640),
            "model_type": "F1 with Endframe",
            "end_frame_strength": job_params.get('end_frame_strength', 0.5)
        }
