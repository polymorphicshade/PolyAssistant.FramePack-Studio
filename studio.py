...
def monitor_job(job_id=None):
    ...
    while True:
        ...
        if job.status == JobStatus.PENDING:
            ...
            yield job.result, last_preview, 'Completed', make_progress_bar_html(100, 'Completed'), gr.update(interactive=True), gr.update(interactive=True, value="Cancel Current Job", visible=False)
            break

        elif job.status == JobStatus.FAILED:
            # Show error and reset the button text
            yield job.result, last_preview, f'Error: {job.error}', make_progress_bar_html(0, 'Failed'), gr.update(interactive=True), gr.update(interactive=True, value="Cancel Current Job", visible=False)
            break
